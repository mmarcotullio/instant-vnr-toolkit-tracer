...
#   3. Creates a uv venv and installs Python deps + build-system tools
#   4. Builds tinycudann from source against the installed torch
#   5. Builds the C++ extension (cmake via scikit-build-core)
...
# ── create venv ───────────────────────────────────────────────────────────────
uv venv "$VENV_DIR" --python 3.11 --prompt .
...
echo "[info] Installing inrtoolkit..."
CMAKE_CUDA_ARCHITECTURES="$SM" UV_INDEX="pytorch=$TORCH_INDEX" TCNN_CUDA_ARCHITECTURES="$SM" \
